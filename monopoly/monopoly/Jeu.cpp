...
void Jeu::lancerPartie()
{
	...
	for (int i = 0; i < nbJoueurs; i++) {
		...
		if (joueurs[i].getMode() == "Aléatoire"){
			std::cout << "Le joueur " << joueurs[i].getPseudo() << " lance les des" << endl;
			int de1 = getRandomNumber();
			int de2 = getRandomNumber();
			std::cout << "De 1 : " << de1 << " ; de 2 : " << de2 << endl;
			int total = de1 + de2;
		...
		if (joueurs[i].getMode() == "Automatique"){
			std::cout << "Le joueur " << joueurs[i].getPseudo() << " lance les des" << endl;
			int de1 = getRandomNumber();
			int de2 = getRandomNumber();
			std::cout << "De 1 : " << de1 << " ; de 2 : " << de2 << endl;
...
void Jeu::lancerDe(Joueur* player)
{
	do {
		...
		if (de1 == de2) {
			...
			player->has_a_double(-player->howManyDoubles()); //retour a 0 !
		}
...
void Jeu::jouerTour(int index)
{
	...
	do {
		...
		std::cout << "Choisis ton option : "<< std::endl;

		if (joueurs[index].getMode() == "Manuel"){
			std::string choice;
			std::cin >> choice;
			char c = choice[0];
		...
		if (joueurs[index].getMode() == "Aléatoire"){
			std::cout << "Le joueur " << joueurs[index].getPseudo() << " effectue une action au hasard. " << std::endl;
			int action = getRandomNumber();
			std::cout << "Action numéro :" << action << endl;
			switch (action) {
			case 1:
				if (!diceRolled) {
					lancerDe(player);
		...
		if (joueurs[index].getMode() == "Automatique"){
			std::cout << "Le joueur " << joueurs[index].getPseudo() << " réfléchit... " << std::endl;
			int action = 1;//A modifier plus tard, pour l'instant on ne fait que jouer les dès
			std::cout << "Action numéro :" << action << endl;
			switch (action) {
			case 1:
				if (!diceRolled) {
